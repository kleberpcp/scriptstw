...
(function(){
  ...
  // 1) Captura imediata do game_data (window.game_data ou variável global game_data)
  // 2) Intercepta TribalWars.updateGameData(...) para salvar player.id e player.name
  // 3) Extrai do DOM do header (seletor seguro do nome do jogador)
  // 4) Extrai do HTML dos <script> inline (regex no updateGameData)
  // 5) Cache persistente (localStorage) com TTL longo
  // 6) (Fallback) fetch de info_player&id=<playerId> para obter nome
  //
  // Importante: removido fallback por document.title, pois ele frequentemente contém o nome/coord da aldeia e não do player.

  const PLAYER_CACHE_TTL = 1000 * 60 * 60 * 24 * 30; // 30 dias

  let _playerNamePromise = null;
  let _playerNameCached = null;

  function normalizePlayerName(raw){
    const s = String(raw || '').replace(/\s+/g,' ').trim();
    if(!s) return null;

    let t = s.replace(/^(?:Perfil\s+de|Profile\s+of)\s+/i,'').trim();
    t = t.replace(/\s*\(\d+\)\s*$/,'').trim();

    const low = t.toLowerCase();
    const badFragments = [
      'continente', 'visualizar', 'comandos agendados', 'tribal wars', 'mundo', 'world',
      'mapa', 'map', 'visão geral', 'overview', 'perfil', 'profile', 'config', 'settings',
      'village', 'aldeia'
    ];
    if(!t || t.length < 2 || t.length > 60) return null;
    if(badFragments.some(f => low.includes(f))) return null;
  ...
  function readPlayerCache(){
    try{
      ...
      if(st.ts && (Date.now() - st.ts) > PLAYER_CACHE_TTL) return null;
      return st;
    }catch{ return null; }
  }
  ...
  function writePlayerCache(id, name){
    ...
    if(!n) return;
    try{
      const pid = id ? String(id) : (getPlayerId() || '');
      localStorage.setItem(PLAYER_CACHE_KEY, JSON.stringify({ id: pid, name: n, ts: Date.now() }));
    }catch{}
    _playerNameCached = n;
  }

  function getGameDataObj(){
    try{ if(window.game_data) return window.game_data; }catch{}
    try{ if(typeof game_data !== 'undefined') return game_data; }catch{}
    return null;
  }

  function capturePlayerFromGameData(){
    try{
      const gd = getGameDataObj();
      if(gd && gd.player && gd.player.name){
        const n = normalizePlayerName(gd.player.name);
        if(n){
          writePlayerCache(gd.player.id, n);
          return n;
        }
      }
    }catch{}
  ...
  function capturePlayerFromHeaderDom(doc){
    const d = doc || document;
    const selectors = [
      '#header_info #player_name',
      '#player_name',
      '#header_info a[href*="screen=info_player"]',
      'a[href*="screen=info_player"][href*="id="]'
    ];
    for(const sel of selectors){
      const el = d.querySelector(sel);
      if(!el) continue;
      const n = normalizePlayerName(el.textContent);
      if(n){
  ...
  function capturePlayerFromInlineScripts(){
    try{
      ...
      for(const s of scripts){
        ...
        // Ex.: TribalWars.updateGameData({"player":{"id":919709026,"name":"Tito",
        const m = txt.match(/updateGameData\(\{\s*"player"\s*:\s*\{\s*"id"\s*:\s*(\d+)\s*,\s*"name"\s*:\s*"([^"]+)"/i);
        if(m && m[2]){
          const n = normalizePlayerName(m[2]);
          if(n){
            writePlayerCache(m[1], n);
            return n;
          }
        }
      }
  ...
  function installUpdateGameDataHook(){
    try{
      if(!window.TribalWars || typeof window.TribalWars.updateGameData !== 'function') return;
      if(window.TribalWars.__mm_hooked) return;

      const orig = window.TribalWars.updateGameData;
      window.TribalWars.updateGameData = function(data){
        try{
          const p = data && data.player ? data.player : null;
          if(p && p.name){
            const n = normalizePlayerName(p.name);
            if(n) writePlayerCache(p.id, n);
          }
        }catch{}
        return orig.apply(this, arguments);
      };
      window.TribalWars.__mm_hooked = true;
    }catch{}
  }

  async function fetchPlayerNameFromInfoPlayer(){
    try{
      const v = getVillageId();
      const pid = getPlayerId() || readPlayerCache()?.id || '';
      if(!pid) return null;

      let url = location.origin + '/game.php?screen=info_player&id=' + encodeURIComponent(pid);
      if (v) url += '&village=' + encodeURIComponent(v);

      const html = await fetch(url, { credentials: 'include' }).then(r=>r.text());
      const doc = new DOMParser().parseFromString(html, 'text/html');

      const n1 = (doc.querySelector('#player_info a') ? doc.querySelector('#player_info a').textContent : '');
      const n2 = (doc.querySelector('#header_info #player_name') ? doc.querySelector('#header_info #player_name').textContent : '');
      const n = normalizePlayerName(n1) || normalizePlayerName(n2);
      if(n) return n;
    }catch{}
    return null;
  }

  function getPlayerNameQuick(){
    const g = capturePlayerFromGameData();
    if(g) return g;

    if(_playerNameCached) return _playerNameCached;

    const cache = readPlayerCache();
    if(cache?.name){
      const n = normalizePlayerName(cache.name);
      if(n){ _playerNameCached = n; return n; }
    }

    const hd = capturePlayerFromHeaderDom(document);
    if(hd) return hd;

    const inl = capturePlayerFromInlineScripts();
    if(inl) return inl;

    return null;
  }

  async function ensurePlayerName(timeoutMs=8000){
    const quick = getPlayerNameQuick();
    if (quick) return quick;

    if (_playerNamePromise){
      const r = await Promise.race([
        _playerNamePromise,
        new Promise(resolve=>setTimeout(()=>resolve(null), timeoutMs))
      ]);
      return r || (getPlayerNameQuick() || 'Desconhecido');
    }

    _playerNamePromise = (async()=>{
      // hook + espera curta para updateGameData
      installUpdateGameDataHook();
      for(let i=0;i<10;i++){
        const q = getPlayerNameQuick();
        if(q) return q;
        await sleep(200);
      }
      const fetched = await fetchPlayerNameFromInfoPlayer();
      if(fetched){
        writePlayerCache(getPlayerId(), fetched);
        return fetched;
      }
      return getPlayerNameQuick() || 'Desconhecido';
    })();
    ...
    return res || (getPlayerNameQuick() || 'Desconhecido');
  }
  ...
  function prefetchPlayerName(){
    installUpdateGameDataHook();
    capturePlayerFromGameData();
    capturePlayerFromHeaderDom(document);
    capturePlayerFromInlineScripts();
    ensurePlayerName(8000).catch(()=>{});
  }
...
function checkExchangeRotation(remainingMs){
  ...
  async function notifyCaptchaOnce(reason){
  ...
  function createUI(){
    ...
    document.getElementById('mm-discord-test').onclick=async()=>{
  ...
  // Objetivo: ao identificar NOVO(S) ataque(s) chegando, enviar notificação no Discord com:
  // - total de ataques chegando
  // - etiqueta (unidade mais lenta) do(s) novo(s) ataque(s), quando possível
  //
  // Obs.: Para descobrir a unidade mais lenta, buscamos os IDs dos comandos em
  // overview_villages -> incomings -> attacks, e para cada novo ID abrimos info_command
  // para obter origem/destino e horários (start/arrival), calculando min/campo e comparando
  // com as speeds oficiais do mundo via interface.php.

  const INCOMINGS_IDS_MAX = 300; // segurança
  const UNIT_CACHE_KEY = 'mm_unit_speed_cache_v1';
  const UNIT_CACHE_TTL = 1000 * 60 * 60 * 12; // 12h
  ...
  function readIncomingsState(){
    ...
    if(!raw) return { last: null, ids: [], ts: 0, lockTs: 0 };
    try{
      const st = JSON.parse(raw) || {};
      return {
        last: (st.last===0 || st.last) ? st.last : null,
        ids: Array.isArray(st.ids) ? st.ids : [],
        ts: st.ts || 0,
        lockTs: st.lockTs || 0
      };
    } catch {
      return { last: null, ids: [], ts: 0, lockTs: 0 };
    }
  }
  ...
  function twCoordToXY(coord){
    const m = String(coord||'').match(/(\d{1,3})\|(\d{1,3})/);
    if(!m) return null;
    return { x: parseInt(m[1],10), y: parseInt(m[2],10) };
  }

  function distCoords(a,b){
    const A = twCoordToXY(a), B = twCoordToXY(b);
    if(!A||!B) return null;
    const dx = A.x - B.x, dy = A.y - B.y;
    return Math.sqrt(dx*dx + dy*dy);
  }

  function parseTwDateTime(raw){
    const s = String(raw||'').replace(/\s+/g,' ').trim();
    if(!s) return null;
    ...
    // usa horário local do browser (mesmo do jogo)
  ...
  function getWorldId(){
    try{
      if(window.game_data && window.game_data.world) return String(window.game_data.world);
    }catch{}
    // fallback: br139.tribalwars.com.br => "br139"
    try{
      const host = location.hostname.toLowerCase();
      const m = host.match(/^(br\d+)\./);
      return m ? m[1] : (`br${BR_NUMBER}`);
    }catch{}
    return `br${BR_NUMBER}`;
  }

  async function getUnitSpeedMap(){
    // cache por mundo
    const world = getWorldId();
    const cacheKey = `${UNIT_CACHE_KEY}::${world}`;
    try{
      const raw = localStorage.getItem(cacheKey);
      if(raw){
        const st = JSON.parse(raw);
        if(st && st.ts && (Date.now()-st.ts) < UNIT_CACHE_TTL && st.speeds && st.unitSpeedFactor){
          return st;
        }
      }
    }catch{}

    // Busca infos oficiais
    const base = location.origin;
    const configXml = await fetchText(base + '/interface.php?func=get_config');
    const unitXml = await fetchText(base + '/interface.php?func=get_unit_info');

    // unit_speed (multiplicador de velocidade de tropas)
    let unitSpeedFactor = 1;
    try{
      const m = String(configXml).match(/<unit_speed>([^<]+)<\/unit_speed>/i);
      if(m) unitSpeedFactor = parseFloat(m[1]);
      if(!Number.isFinite(unitSpeedFactor) || unitSpeedFactor<=0) unitSpeedFactor = 1;
    }catch{ unitSpeedFactor = 1; }

    // speeds por unidade (min/campo "base", que deve ser ajustado por unit_speed)
    // Observação: em get_unit_info, <speed> costuma ser em min/campo no mundo "1x".
    const speeds = {};
    const re = /<(\w+)>\s*<speed>([^<]+)<\/speed>/gi;
    let mm;
    while((mm = re.exec(String(unitXml))) !== null){
      const unit = mm[1];
      const sp = parseFloat(mm[2]);
      if(Number.isFinite(sp)) speeds[unit] = sp;
    }

    const st = { ts: Date.now(), world, unitSpeedFactor, speeds };
    try{ localStorage.setItem(cacheKey, JSON.stringify(st)); }catch{}
    return st;
  }

  function effectiveMinPerField(unit, unitInfo){
    const base = unitInfo?.speeds?.[unit];
    const f = unitInfo?.unitSpeedFactor || 1;
    if(!Number.isFinite(base)) return null;
    // quanto maior unit_speed, mais rápido; então min/campo efetivo diminui
    return base / f;
  }

  function pickClosestUnit(observedMinPerField, unitInfo){
    const candidates = [
      'snob','catapult','ram',
      'heavy','light','knight',
      'spy','marcher','archer',
    ...
    for(const u of candidates){
      const mpf = effectiveMinPerField(u, unitInfo);
      if(!mpf) continue;
      const diff = Math.abs(mpf - observedMinPerField) / mpf; // erro relativo
      if(best===null || diff < best.diff){
        best = { unit: u, mpf, diff };
      }
    }
    // tolerância (se for muito fora, retorna null)
    if(!best) return null;
    if(best.diff > 0.22) return null; // 22% costuma separar bem (nobre/ram/cata etc)
    return best.unit;
  }

  function unitToLabel(unit){
    const map = {
      snob: { name:'Nobre', emoji:'👑' },
      ram: { name:'Ariete', emoji:'🪵' },
      catapult: { name:'Catapulta', emoji:'🪨' },
      spy: { name:'Explorador', emoji:'🕵️' },
      heavy: { name:'Cavalaria pesada', emoji:'🐴' },
      light: { name:'Cavalaria leve', emoji:'🏇' },
      knight: { name:'Paladino', emoji:'⚔️' },
      axe: { name:'Machado', emoji:'🪓' },
      sword: { name:'Espada', emoji:'🗡️' },
      spear: { name:'Lança', emoji:'🔱' },
      archer: { name:'Arqueiro', emoji:'🏹' },
      marcher: { name:'Arqueiro a cavalo', emoji:'🏹' }
    };
    return map[unit] || { name:'Desconhecido', emoji:'❓' };
  }

    function findCoordByLabel(doc, labels){
    ...
    for(const tr of rows){
      ...
      if(!th || !td) continue;
      const lab = String(th.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();
      if(!labs.some(x => lab === x || lab.startsWith(x))) continue;

      const coordEl = td.querySelector('.coord') || td.querySelector('span[class*="coord"]');
      if(coordEl){
        const m = String(coordEl.textContent||'').match(/\b\d{1,3}\|\d{1,3}\b/);
        if(m) return m[0];
      }
      const m2 = String(td.textContent||'').match(/\b\d{1,3}\|\d{1,3}\b/);
      if(m2) return m2[0];
    }
    return null;
  }

  function findCellValueByLabel(doc, labels){
    const labs = labels.map(l=>String(l).toLowerCase());
    const rows = [...doc.querySelectorAll('tr')];
    for(const tr of rows){
      const th = tr.querySelector('th');
      ...
      const lab = String(th.textContent||'').replace(/\s+/g,' ').trim().toLowerCase();
      if(!labs.some(x => lab === x || lab.startsWith(x))) continue;
      const val = String(td.textContent||'').replace(/\s+/g,' ').trim();
      if(val) return val;
    }
    return null;
  }

  function extractOriginTargetFromCommand(doc){
    const from = findCoordByLabel(doc, ['origem','origin','from']);
    const to   = findCoordByLabel(doc, ['destino','target','to']);
    if(from && to) return { from, to };

    const txt = String(doc.body?.textContent || '');
    const all = (txt.match(/\b\d{1,3}\|\d{1,3}\b/g) || []).map(x=>x.trim());
    const uniq = [];
    for(const c of all){
      if(!uniq.includes(c)) uniq.push(c);
      if(uniq.length>=2) break;
    }
    return uniq.length>=2 ? { from: uniq[0], to: uniq[1] } : null;
  }

    async function tagIncomingCommand(commandId){
    try{
      const v = getVillageId();
      const base = location.origin;
      let url = base + '/game.php?screen=info_command&id=' + encodeURIComponent(commandId);
      if(v) url += '&village=' + encodeURIComponent(v);

      const html = await fetchText(url);
      const doc = new DOMParser().parseFromString(html, 'text/html');

      const sentRaw = findCellValueByLabel(doc, ['Enviado', 'Sent']);
      const arrRaw  = findCellValueByLabel(doc, ['Chegada', 'Arrival', 'Chegará', 'Chega']);
      const sent = parseTwDateTime(sentRaw);
      const arr  = parseTwDateTime(arrRaw);

      let durationMs = null;
      if(sent && arr){
        durationMs = arr.getTime() - sent.getTime();
      } else {
        const travRaw = findCellValueByLabel(doc, ['Tempo de viagem', 'Travel time']);
        if(travRaw){
          const m = String(travRaw).match(/(\d+):(\d{2})(?::(\d{2}))?/);
          if(m){
            const h = parseInt(m[1],10), mi=parseInt(m[2],10), se=parseInt(m[3]||'0',10);
            durationMs = ((h*3600)+(mi*60)+se)*1000;
          }
        }
      }
      if(!durationMs || durationMs<=0) return { unit:null, label:null };

      const coords = extractOriginTargetFromCommand(doc);
      if(!coords) return { unit:null, label:null };

      const d = distCoords(coords.from, coords.to);
      if(!d || d<=0.01) return { unit:null, label:null };

      const observed = (durationMs/60000) / d;

      const unitInfo = await getUnitSpeedMap();
      const u = pickClosestUnit(observed, unitInfo);
      if(!u) return { unit:null, label:null };

      const lab = unitToLabel(u);
      return { unit:u, label:lab, from:coords.from, to:coords.to, observedMinPerField:observed };
    }catch{
      return { unit:null, label:null };
    }
  }

  async function fetchIncomingCommandIds(){
    try{
      const v = getVillageId();
      const base = location.origin;
      let url = base + '/game.php?screen=overview_villages&mode=incomings&subtype=attacks';
      if(v) url += '&village=' + encodeURIComponent(v);

      const html = await fetchText(url);
      // IDs estão nos links screen=info_command&id=XXXX
      const ids = [];
      const re = /screen=info_command[^"']*?[&?]id=(\d+)/gi;
      let m;
      while((m = re.exec(html)) !== null){
        const id = m[1];
        if(id && !ids.includes(id)) ids.push(id);
        if(ids.length >= INCOMINGS_IDS_MAX) break;
      }
      return ids;
    }catch{
      return [];
  ...
  async function checkAndNotifyIncomings(){
    ...
    // Primeira leitura: baseline (evita notificar na instalação)
    if(st.last === null){
      // tenta coletar ids também (melhor baseline)
      const ids = await fetchIncomingCommandIds();
      writeIncomingsState({ last: count, ids, ts: Date.now(), lockTs: 0 });
      return;
    }

    // Evita duplicidade em múltiplas abas (janela curta de lock)
    const now = Date.now();
    if(st.lockTs && (now - st.lockTs) < 5000) return;

    if(count > st.last){
      // novo(s) ataque(s)
      // Recolhe IDs e identifica quais são novos
      const ids = await fetchIncomingCommandIds();
      const prevSet = new Set((st.ids||[]).map(String));
      const newIds = (ids||[]).map(String).filter(x => x && !prevSet.has(x));

      // atualiza state antes de notificar (evita duplicar em recarregamentos)
      writeIncomingsState({ last: count, ids, ts: now, lockTs: now });

      // etiqueta do(s) novo(s) ataque(s)
      let extra = '';
      if(newIds.length){
        const tags = [];
        for(const id of newIds.slice(0, 5)){ // evita payload gigante
          const t = await tagIncomingCommand(id);
          if(t?.label?.name){
            const L = t.label;
            const coordInfo = (t.from && t.to) ? ` (${t.from} → ${t.to})` : '';
            tags.push(`🏷️ Novo ataque: ${L.emoji} **${L.name}**${coordInfo}`);
          } else {
            tags.push(`🏷️ Novo ataque: ❓ **Desconhecido** (id ${id})`);
          }
        }
        if(newIds.length > 5) tags.push(`➕ +${newIds.length-5} ataque(s) novo(s)`);
        extra = tags.join('\n');
      } else {
        extra = '🏷️ Novo ataque: ❓ **Desconhecido**';
      }

      const details = [
        `⚔️ Ataques chegando: **${count}**`,
        extra
      ].filter(Boolean).join('\n');

      // Evita embed preview: URL entre <...>
      ...
      const ids = await fetchIncomingCommandIds().catch(()=>[]);
      writeIncomingsState({ last: count, ids: ids||[], ts: now, lockTs: 0 });
    }
  }
  ...
  function safeBoot(){
    try{
      ...
      // Aquecer cache do nome do player o quanto antes (para notificações)
      prefetchPlayerName();
